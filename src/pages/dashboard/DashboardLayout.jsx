...
const DashboardLayout = () => {
    ...
    useEffect(() => {
        if (userData.role === "instructor") {
            setFields([
                {
                    name: "DashBoard",
                    slug: "/user/InstructorDashboard",
                    icon: faBorderAll,
                },
                {
                    name: "My Courses",
                    slug: "/user/InstructorDashboard/myCourses",
                ...
                {
                    name: "Edit Profile",
                    slug: "/user/InstructorDashboard/editProfile",
                    icon: faPencilSquare,
                },
    ...
    return (
        <div className="mt-5 font-medium">
                ...
                <div className="flex gap-4 items-center">
                  ...
                  // {/* reviews end s */}
                            ...
                            <div>
                                ...
                                />
                                <span className="font-bold ">{userCourses?.length}</span>{" "}
                                {/* backend data */}
                                courses
                            </div>
                            {/* courses end s */}
            ...
            {/* top ends  */}
            <main className="flex gap-2 lg:grid lg:grid-cols-4 lg:gap-7 mt-7 mb-10">
                <section className="leftSide w-12 lg:w-auto">
                    <div className=" w-full flex flex-col gap-[10px] p-1 lg:p-5 bg-primary-light rounded-md">
                        {fields.map((field, index) => (
                            <NavLink
                                ...
                                className={({ isActive }) =>
                                    ...
                                    } flex justify-center lg:justify-start gap-2 items-center py-2 px-4 transition hover:scale-105 duration-500 text-primary hover:text-white hover:bg-primary rounded-md dark:text-white `
                                }
                                key={index}
                            >
                                <FontAwesomeIcon icon={field.icon} />
                                <div className="hidden lg:block">{field.name}</div>
                            </NavLink>
                        ))}
                        <Button
                            className={` flex  gap-2 items-center py-2 !px-4 !transition hover:scale-105  duration-500 text-primary hover:text-white hover:bg-primary rounded-md dark:text-white outline-none border-none lg:!justify-start`}
